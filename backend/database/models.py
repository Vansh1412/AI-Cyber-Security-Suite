...
scheduler_state          — singleton row tracking distributed scheduler epoch (Phase 5)
...
class MonitoringTarget(Base):
    ...
    execution_epoch        = Column(Integer, nullable=True)        # Epoch at claim time
    execution_expires_at   = Column(DateTime(timezone=True), nullable=True)  # Lease expiry (DB NOW-based)
...
class SchedulerState(Base):
    """
    Singleton table (always id=1) that stores the distributed scheduler
    leader-election epoch.  Every leader election increments current_epoch
    atomically via a conditional UPDATE, providing a monotonic fencing token
    that prevents split-brain and stale worker writes.

    PostgreSQL: pg_try_advisory_lock + epoch UPDATE combination.
    SQLite dev: single-process; advisory lock emulated as always-true.
    """
    ...
    leader_pod_id     = Column(String(128), nullable=True)
